...
class ArkoudaMetrics:
    ...
    def __init__(
        ...
        arkoudaMetricsPort: int,
        exportPort: int = 5080,
        serverName: str = "arkouda-metrics",
    ...
    def _updatePerUserNumberOfRequests(self, metric: UserMetric) -> None:
        ...
        if metricName == "total":
            self.perUserTotalNumberOfRequests.labels(
                user=metric.user, arkouda_server_name=self.serverName
            ).set(metric.value)
        else:
    ...
    def _updateGlobalNumberOfRequests(self, metric: Metric) -> None:
        ...
        if metricName == "total":
            self.totalNumberOfRequests.labels(arkouda_server_name=self.serverName).set(
                metric.value
            )
        else:
            self.numberOfRequestsPerCommand.labels(
                command=metricName, arkouda_server_name=self.serverName
            ).set(metric.value)

    def _updateResponseTimes(self, metric: Metric) -> None:
        self.responseTimesPerCommand.labels(
            command=metric.name, arkouda_server_name=self.serverName
        ).set(metric.value)

    def _updateServerMetrics(self, metric: Metric) -> None:
        self.numberOfConnections.labels(arkouda_server_name=self.serverName).set(
            metric.value
        )
